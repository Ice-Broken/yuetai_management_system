<template>
  <div class="box">
    <el-input placeholder="请输入账号" v-model="user" clearable prefix-icon="el-icon-user">
    </el-input>
    <el-input placeholder="请输入密码" v-model="password" show-password clearable prefix-icon="el-icon-key"
      style="margin-top: 20px"></el-input>
    <el-select v-model="id" placeholder="请选择">
      <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
      </el-option>
    </el-select>
    <el-button type="primary" icon="el-icon-s-promotion" @click="logn">登录</el-button>
  </div>
</template>
<script setup>
