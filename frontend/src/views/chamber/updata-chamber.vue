...
import readUser from "@/modules/common/read-chamber";
let route = useRoute();
...
readUser({
  ...
  let data = res.res[0];
  chamber_name_ch.value = data.chamber_name_ch;
...
function create() {
  ...
  if (isCreate) {
    api(
      ` UPDATE prod_chamber SET remark='${remark.value}', time = CURRENT_TIMESTAMP WHERE chamber_name_ch='${chamber_name_ch.value}';`
    ).then((res) => {
      if (res.res) {
        ElMessage({
          message: `修改成功`,
