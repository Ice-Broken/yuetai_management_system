<template>
    ...
    <el-input placeholder="请输入商会名称" v-model="compName" class="company_name" clearable v-if="isAdmin">
    </el-input>
    <el-button icon="el-icon-search" @click="search" class="search-btn" circle v-if="isAdmin"></el-button>
    &nbsp;&nbsp;&nbsp;&nbsp; 
    <el-button type="primary" icon="el-icon-plus" @click="upload"> 新增商会 </el-button>
    <el-table :data="tableData" style="width:100%" :default-sort="{ prop: 'time', order: 'descending' }">
        <el-table-column prop="chamber_name_ch" label="商会中文名称" width="160" text-align="center" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="chamber_name_en" label="商会英文名称" width="160" text-align="center" show-overflow-tooltip>
        </el-table-column>
        ...
        <el-table-column label="删除" v-if="isTeacher != 'teacher'">
            <template v-slot="scope">
                <el-button type="danger" icon="el-icon-delete" circle
                    @click="remove(scope.row.chamber_name_ch)"></el-button>
            </template>
        </el-table-column>
...
api(`SELECT DISTINCT * FROM prod_chamber`).then((res) => {
    data.value = res.res;
...
function change(data) {
    if (localStorage.id == "admin") {
        router.push({ path: "/updata-chamber", query: { id: data.row.chamber_name_ch } });
    }
}

function detail(data) {
    if (localStorage.id == "admin") {
        router.push({ path: "/detail-chamber", query: { id: data.row.chamber_name_ch } });
    }
}

function upload() {
    router.push({ path: "/create-chamber"});
}

function remove(id, index) {
    api(`DELETE FROM prod_chamber WHERE chamber_name_ch='${id}';`).then((res) => {
        router.go(0);
    });
}
...
function selectSub() {
    api(`select * from prod_chamber where subject='${stuClass.value}';`).then((res) => {
        data.value = res.res;
    });
}
...
let compName = ref("");
function search() {
    let sql = `SELECT * FROM prod_chamber
    WHERE chamber_name_ch LIKE '%${compName.value}%'
    OR chamber_name_en LIKE '%${compName.value}%';`;
    api(sql).then((res) => {
        data.value = res.res;
    });
}
